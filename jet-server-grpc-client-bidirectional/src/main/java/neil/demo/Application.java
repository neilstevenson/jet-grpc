...
public class Application {
    ...
    private static Pipeline build(boolean retry) throws Exception {
        ...
        .mapUsingServiceAsyncBatched(getBidirectionalService(retry),
    ...
    private static ServiceFactory<?, ? extends GrpcService<InputMessage, OutputMessage>>
        getBidirectionalService(boolean retry) {
        return GrpcServices.bidirectionalStreamingService(
                        () -> MyUtils.getManagedChannelBuilder(MyConstants.MY_STREAMING_CALL, retry),
                        channel -> WordCountGrpc.newStub(channel)::myStreamingCall
                );
    }
